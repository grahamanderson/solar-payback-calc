...
/*
 * My Amendment notes:  To call the amended class, use the below
    ...
    */
    ...
    private function single_curl() {
        ...
        if ($this->callback) {
            ...
            * if (is_callable($this->callback))$callback($output, $info); 
            */
        	 //-------------My Amendments---------------------/
            	$this->evalParentFunction($this->callback);
             //----------------------------------------------/
        } else {
            return $output;
        }
    ...
    private function rolling_curl($window_size = null) {    
        ...
        do {
            ...
            while($done = curl_multi_info_read($master)) {
                ...
                /*
                 * Original Code
                 * if (is_callable($callback)) $callback($output, $info);
				*/
...
//----------------------------------------------------------------------------
	...
	//---------------------My Ammendments-----------------------------/
	private  function evalParentFunction($callback){		
        # Process the callback function for eval
        $callback = str_replace('$this->','',$callback);
                
        # Return True and to test the eval function...
        $callback .= ';return TRUE;'; 
        $callback = '$this->parentClass->'.$callback; 
  
        # Did the function properly run?
        if (  FALSE === eval( $callback) ):
       		 // Error.
       		 die('Something went awry. The callback function syntax is probably incorrect ');
   		endif;
	}     	
